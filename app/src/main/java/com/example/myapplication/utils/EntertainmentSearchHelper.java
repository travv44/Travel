...
public class EntertainmentSearchHelper {
    ...
    private String generateStaticMapUrl(String lat, String lon) {
        // Генерируем URL для статической карты с меткой места (улучшенное качество)
        // Используем больший размер и более высокий зум для лучшего качества изображения
        return "https://static-maps.yandex.ru/1.x/?ll=" + lon + "," + lat + 
               "&z=16&size=600,400&pt=" + lon + "," + lat + ",pm2rdm&l=map";
    }
    ...
    /**
     * Быстрый подбор мест рядом с пользователем: мало HTTP-запросов, результаты сортируются по расстоянию.
     */
    public List<EntertainmentPlace> searchNearbyPicks(String userLat, String userLon, int maxResults) {
        List<EntertainmentPlace> out = new ArrayList<>();
        if (maxResults <= 0 || userLat == null || userLon == null
                || userLat.trim().isEmpty() || userLon.trim().isEmpty()) {
            return out;
        }
        String city = getLocationName(userLat, userLon);
        if (city == null || city.isEmpty()) {
            List<EntertainmentPlace> fallback = searchByLocation(userLat, userLon);
            sortByDistanceTo(fallback, userLat, userLon);
            return fallback.size() > maxResults ? new ArrayList<>(fallback.subList(0, maxResults)) : fallback;
        }

        String[] queries = new String[]{
                city + " музей",
                city + " парк",
                city + " достопримечательность",
                city + " смотровая",
                city + " набережная"
        };
        ...
        for (String q : queries) {
            if (out.size() >= maxResults + 8) {
                break;
            }
            try {
                String url = YANDEX_GEOCODER_API + "?apikey=" + YANDEX_API_KEY
                        + "&geocode=" + URLEncoder.encode(q, StandardCharsets.UTF_8.name())
                        + "&format=json&results=6&lang=ru_RU";
                Request request = new Request.Builder().url(url).build();
                try (Response response = client.newCall(request).execute()) {
                    if (response.isSuccessful() && response.body() != null) {
                        String json = response.body().string();
                        List<EntertainmentPlace> batch = parseGeocoderResponse(json, userLat, userLon);
                        for (EntertainmentPlace p : batch) {
                            String key = p.getName() != null
                                    ? p.getName().toLowerCase(Locale.ROOT).trim()
                                    : "";
                ...
                Log.w(TAG, "searchNearbyPicks query failed: " + q, e);
            }
        }

        sortByDistanceTo(out, userLat, userLon);
    ...
    private void sortByDistanceTo(List<EntertainmentPlace> list, String uLat, String uLon) {
        try {
            double ulat = Double.parseDouble(uLat.trim().replace(',', '.'));
            double ulon = Double.parseDouble(uLon.trim().replace(',', '.'));
            list.sort(Comparator.comparingDouble(p -> {
                try {
                    double plat = Double.parseDouble(p.getLat().replace(',', '.'));
                    double plon = Double.parseDouble(p.getLon().replace(',', '.'));
                    return calculateDistance(ulat, ulon, plat, plon);
                } catch (Exception e) {
                    return 10_000;
                }
            }));
